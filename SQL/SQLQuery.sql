...
WHERE 
    ...
    MONTH(order_timestamp) BETWEEN 1 AND 9
GROUP BY 
...
ORDER BY 
    ...
    -- Finding monthwise, delivery time. cancelled orders, ratings and others

    -- Orders, Cancellation Rate, Ratings, and Delivery Time (for months 1-9)

SELECT 
    MONTH(fact_orders.order_timestamp) AS order_month, 
    COUNT(fact_orders.order_id) AS total_orders,
    COUNT(CASE WHEN fact_orders.is_cancelled = 1 THEN 1 END) AS cancelled_orders,
    COUNT(fact_ratings.rating) AS total_ratings,
    AVG(fact_ratings.rating) AS avg_rating,
    AVG(fact_delivery_performance.actual_delivery_time_mins) AS avg_delivery_time
FROM 
    fact_orders
LEFT JOIN 
    fact_ratings ON fact_orders.order_id = fact_ratings.order_id
LEFT JOIN 
    fact_delivery_performance ON fact_orders.order_id = fact_delivery_performance.order_id
WHERE 
    YEAR(fact_orders.order_timestamp) = 2025 AND 
    MONTH(fact_orders.order_timestamp) BETWEEN 1 AND 9
GROUP BY 
    MONTH(fact_orders.order_timestamp)
ORDER BY 
    order_month;
    ...
    SELECT 
    CASE 
        WHEN MONTH(fact_orders.order_timestamp) BETWEEN 1 AND 5 THEN 'Pre-Crisis'
        WHEN MONTH(fact_orders.order_timestamp) BETWEEN 6 AND 9 THEN 'Post-Crisis'
    END AS period,
    COUNT(fact_orders.order_id) AS total_orders,
    COUNT(CASE WHEN fact_orders.is_cancelled = 1 THEN 1 END) AS cancelled_orders,
    COUNT(fact_ratings.rating) AS total_ratings,
